...
a <-Sys.time()
dups3orig <- ddply(dups3, 1, get_lags_leads_missing_data, n)
b <- Sys.time()
print(b-a) #35.1286 secs

dups3$row_ID <- 1:nrow(dups3)
a <- Sys.time()
dups3orig2 <- dups3 %>%
        dplyr::group_by(row_ID) %>%
        dplyr::do(get_lags_leads_missing_data(., n)) %>%
        dplyr::ungroup()
b <- Sys.time()
print(b-a) #1.086239 mins
...
dups3origa <- replace_missing_data(dups3orig, n)
dups3origb <- replace_missing_data(dups3orig2, n)
...
sum(is.na(dups3origb$start_date_new)) #15 NAs - 199 mods
#END DATES
